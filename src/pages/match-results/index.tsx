...
import { getMatchResults, deleteMatchResultById } from 'apiSdk/match-results';
import { MatchResultInterface } from 'interfaces/match-result';

type ColumnType = ColumnDef<MatchResultInterface, unknown>;

interface MatchResultListPageProps {
...
export function MatchResultListPage(props: MatchResultListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MatchResultInterface>>(
    () => `/match-results?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MatchResultInterface) => {
    if (hasAccess('match_result', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/match-results/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/match-results/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('match_result', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/match-results/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="match_result" />
        </Flex>

        {hasAccess('match_result', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/match-results/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
