...
import { getBetHistories } from 'apiSdk/bet-histories';
import { BetHistoryInterface } from 'interfaces/bet-history';
import { getGameMatches } from 'apiSdk/game-matches';
...
import { MatchResultListPage as ShowCaseList } from 'pages/match-results';

function DashboardPage() {
  const { session } = useSession();

  const betHistoryStat = useSWR<PaginatedInterface<BetHistoryInterface>>(
    () => `/bet-histories`,
    () => getBetHistories({ limit: 1, offset: 0 }),
  );

  const gameMatchStat = useSWR<PaginatedInterface<GameMatchInterface>>(
    () => `/game-matches`,
    () => getGameMatches({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Bet Histories',
      result: betHistoryStat?.data?.totalCount,
      isLoading: betHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Game Matches',
      result: gameMatchStat?.data?.totalCount,
      isLoading: gameMatchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Profiles',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`match-results`} passHref legacyBehavior>
                  <Button
